...
class OperatorDiGraph():
    def __init__(self,sas=None):
        ...
        for op in sas.operators:
            effect_var = sorted({var for var,value in op.effect.items() if sas.is_essential(var)})
            requirement_var = sorted({var for var,value in op.requirement.items() if sas.is_essential(var)})
